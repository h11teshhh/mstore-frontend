...
class BillPdf {
        ...
        child: pw.Column(children: [
          ...
          pw.Container(
            ...
            child: pw.Row(children: [
              ...
              pw.Container(width: 0.8, height: 35, color: PdfColors.black,
          ...
          pw.Expanded(child: pw.Stack(children: [
            ...
            pw.Positioned.fill(child: pw.Row(children: [
              pw.SizedBox(width: colSl),
              pw.VerticalDivider(width: 0.8, thickness: 0.8, color: PdfColors.black),
            ...
            pw.Container(
              ...
              child: pw.Column(mainAxisSize: pw.MainAxisSize.max, children: [
                ...
                pw.Container(color: PdfColors.grey200, child: pw.Row(children: [
                  _cell('Sl.',    colSl,     baseFont, bold: true, center: true),
                  pw.Expanded(child: _cell('Item', 0, baseFont, bold: true, center: true)),
                  _cell('Qty',    colQty,    baseFont, bold: true, center: true),
                  _cell('Rate',   colRate,   baseFont, bold: true, center: true),
                  _cell('Amount', colAmount, baseFont, bold: true, center: true),
                ])),
                pw.Divider(height: 0.8, thickness: 0.8),
                // Data rows
                ...items.asMap().entries.map((e) => pw.Row(children: [
                  _cell('${e.key + 1}',                     colSl,     baseFont, center: true),
                  pw.Expanded(child: _cell(e.value['item_name'] ?? '', 0, baseFont)),
                  _cell(e.value['quantity'].toString(),      colQty,    baseFont, right: true),
                  _cell(_fmt(e.value['price']  ?? 0),        colRate,   baseFont, right: true),
                  _cell(_fmt(e.value['total']  ?? 0),        colAmount, baseFont, right: true),
                ])),
              ]),
            ),
          ])),
              ...
              pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
                ...
                pw.Padding(
                  ...
                  child: pw.Text('THANK YOU',
                      style: pw.TextStyle(fontSize: baseFont + 2, fontWeight: pw.FontWeight.bold)),
                ),
              ]),
              pw.Container(
                width: 180,
                ...
                decoration: pw.BoxDecoration(border: pw.Border.all(width: 1)),
                child: pw.Column(children: [
                  _totRow('Bill Total',  _fmt(todayBill),  baseFont, bold: true),
                  _totRow('Prev. Due',   _fmt(previousDue),baseFont),
                  pw.Divider(thickness: 1),
                  _totRow('GRAND TOTAL', _fmt(grandTotal), baseFont + 1, bold: true),
                ]),
              ),
            ],
          ),
  ...
  static pw.Document generate({
    ...
    doc.addPage(buildPage(
      area: area, customerName: customerName, customerId: customerId,
      orderId: orderId, phone: phone, billDate: billDate,
      items: items, todayBill: todayBill, previousDue: previousDue,
    ));
    return doc;
  }
  ...
  static pw.Document generateBulk(List<BillSpec> specs) {
    ...
    for (final s in specs) {
      doc.addPage(buildPage(
        area: s.area, customerName: s.customerName, customerId: s.customerId,
        orderId: s.orderId, phone: s.phone, billDate: s.billDate,
        items: s.items, todayBill: s.todayBill, previousDue: s.previousDue,
      ));
    }
    return doc;
  }
  ...
  static pw.Widget _hRow(String label, String value, double size, {bool bold = false}) =>
      pw.Row(children: [
        pw.SizedBox(width: 45, child: pw.Text(label, style: pw.TextStyle(fontSize: size))),
  ...
  static pw.Widget _cell(String t, double w, double s,
  ...
  static pw.Widget _totRow(String l, String v, double s, {bool bold = false}) =>
      pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
        ...
        pw.Text(v, style: pw.TextStyle(fontSize: s,
            fontWeight: bold ? pw.FontWeight.bold : pw.FontWeight.normal)),
      ]);

  static pw.Widget _sigLine(String label, double size) =>
      pw.Column(children: [
        pw.Container(width: 100, decoration: const pw.BoxDecoration(
